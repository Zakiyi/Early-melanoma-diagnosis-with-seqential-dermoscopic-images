...
def get_model_output(model, data):
    ...
    for name, img_seq in data['image'].items():
        preds = OrderedDict({})
        avg, s, t, e = model(img_seq['images'].to(device), img_seq['diff_images'].to(device))

        if isinstance(avg, tuple) or isinstance(avg, list):
            avg = avg[-1].detach().cpu().numpy().mean(axis=0).squeeze()

        s = [x.squeeze().detach().cpu().mean().sigmoid().numpy() for x in s]
        t = [x.squeeze().detach().cpu().mean().sigmoid().numpy() for x in t]
        e = [x.squeeze().detach().cpu().mean().sigmoid().numpy() for x in e]

        preds['avg_score'] = avg
        preds['spatial_score'] = s
        preds['temporal_score'] = t
        preds['exit_score'] = e
...
def evaluation(cfg, model, data_split_file):
    ...
    :return:
    """
    test_augmentor = Augmentations_diff(test_aug_parameters, test_mode=True, color_recalibration=True, test_aug='ten_crops')
    ...
    test_results = OrderedDict({'target': [], 'avg_score': [], 'pred_label': [], 'spatial_score': [], 'temporal_score': [], 'exit_score': []})
    model.eval()
    with torch.no_grad():
        for data in iter(dataset):
            outputs = get_model_output(model, data)

            test_results['spatial_score'].append(outputs['seq_0']['spatial_score'])
            test_results['temporal_score'].append(outputs['seq_0']['temporal_score'])
    ...
    test_results['spatial_score'] = np.stack(test_results['spatial_score'], axis=0)
    test_results['temporal_score'] = np.stack(test_results['temporal_score'], axis=0)
...
def cross_fold_validation(cfg, result_dir, best_model=True, show_roc=True):
    ...
    for i in range(len(folds)):
        ...
        print('current fold is ', folds[i])
        if not os.path.exists(os.path.join(folds[i], 'exit_scores_eval.pt')):
            # load model
...
if __name__ == "__main__":
    ...
    training_result_dirs = '/media/zyi/080c2d74-aa6d-4851-acdc-c9588854e17c/medical_AI/CrossValidation_test/TRNN/round_3'
    cfg['model'] = 'cnn-diff-hc-re'
    seq_length = [4]
    # for training_result_dir in glob(os.path.join(training_result_dirs, 'd*')):
    #     print(training_result_dir)
    ...
    for dir in glob(os.path.join(training_result_dirs, 'd*')):
        ...
        for length in seq_length:
            ...
            with open(res_dir, 'rb') as f:
                m = json.load(f)
            res = [m['time_0']['auc'][0], m['time_1']['auc'][0], m['time_2']['auc'][0], m['time_3']['auc'][0]]
            plt.plot(res, linewidth=1.8)
    plt.legend(legends)
    # plt.savefig(os.path.join(training_result_dirs, 'res.png'))
    plt.show()