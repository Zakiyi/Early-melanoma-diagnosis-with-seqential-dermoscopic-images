...
def evaluation(cfg, model, data_split_file):
    ...
    test_results = OrderedDict({'target': [], 'exit_score': [], 'pred_label': [], 'pred_score': []})
    model.eval()

    with torch.no_grad():
        for data in iter(dataset):
            outputs = get_model_output(model, data, cfg['data']['modality'])
            pred_score = outputs[-1].detach().cpu().sigmoid().numpy()  # (Ncrops * seq_length, 1)
            pred_score = np.mean(pred_score, axis=0)

            print('pred_score: ', pred_score)
            print('target: ', data['target'].detach().cpu().numpy())

            e = [x.squeeze().mean() for x in outputs]
...
if __name__ == "__main__":
    ...
    training_result_dir = '/media/zyi/080c2d74-aa6d-4851-acdc-c9588854e17c/medical_AI/CrossValidation_test/CNN features/CNN_LSTM'
    seq_length = [4]
    cfg['model'] = 'cnn-lstm'
