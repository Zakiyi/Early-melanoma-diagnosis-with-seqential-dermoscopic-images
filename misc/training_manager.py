...
class training_manager():
    ...
    def initialize_model(self, data_channel, n_classes, seq_length, learning_rate,
        ...
        if os.path.exists(fine_tune['pre_trained_model']):
            pass
            # print('we are using pre_trained_baseline!!!')
            # model_state = torch.load(fine_tune['pre_trained_model'], map_location=self.device)['model_state_dict']
            # self.model.load_state_dict(model_state)
            #
            # # load model state dict
            # try:
            #     self.model.load_state_dict(model_state)  # maybe need to change
            # except RuntimeError:
            #     model_state_dict = convert_state_dict(model_state)
        ...
        if fine_tune['stage'] == 'none':
            ...
            if 'cnn-diff' not in self.model_name:
                try:
                    print('we are only training classifier of non cnn-diff')
                    ...
                    self.optimizer = self.optimizer([{'params': self.model.lstm_layer.parameters()},
                                                     {'params': self.model.clss_layer.parameters()}
                                                     ],
                                                    learning_rate, weight_decay=optim_dict['weight_decay'])
            else:
                try:
                    print('nono CPU: for conv-diff we only train the classifier!!!')
                    self.optimizer = self.optimizer([
                                                     {'params': self.model.img_sbn_classifier.parameters()},
                                                     {'params': self.model.dym_dfn_classifier.parameters()},
                                                     {'params': self.model.attention_block.parameters()},
                                                     {'params': self.model.clss_layer.parameters()}
                                                     ], learning_rate,
                                                    weight_decay=optim_dict['weight_decay'])
                except AttributeError:
            ...
            print('stage last: we are using pre-trained classifier!!!')
            ...
            if 'cnn-diff' not in self.model_name:
                ...
                try:
                    print('last: for conv-diff we only train the classifier!!!')
                    self.optimizer = self.optimizer([{'params': self.model.attention_block.parameters()},
                                                     {'params': self.model.clss_layer.parameters()},
                                                     ], learning_rate,
                                                    weight_decay=optim_dict['weight_decay'])
                except AttributeError:
                    print('last cpu: for conv-diff we only train the classifier!!!')
                    self.optimizer = self.optimizer([
                                                     # {'params': self.model.img_sbn_classifier.parameters()},
                                                     # {'params': self.model.dym_dfn_classifier.parameters()},
                                                     {'params': self.model.exit_layer.parameters()},
                                                     {'params': self.model.clss_layer.parameters()},
                                                     ], learning_rate,
                                                    weight_decay=optim_dict['weight_decay'])
        else:
