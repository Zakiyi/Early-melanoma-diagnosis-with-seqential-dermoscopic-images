...
class training_manager():
    def __init__(self, output_dir, model_name, optimizer, lr_scheduler, device, max_epoch):
        ...
        self.monitor_MA_alpha = 0.9  # alpha * old + (1-alpha) * new

    def initialize_model(self, data_channel, n_classes, seq_length, learning_rate,
                         optim_dict, lr_scheduler_dict, device, fine_tune):

        self.model = get_model(self.model_name, data_channel, n_classes, seq_length).to(device)

        if device == torch.device('cuda') and torch.cuda.device_count() > 1:
            print('we are using multiple gpus!!!')
            self.model = torch.nn.DataParallel(self.model)

        if fine_tune['stage'] == 'none':
            ...
            if 'cnn-diff' not in self.model_name:
                # model_state = torch.load(fine_tune['pre_trained_model'], map_location=self.device)['model_state_dict']
                # self.model.load_state_dict(model_state)
                # for para in self.model.module.encoder.parameters():
                #     para.requires_grad = False

                # for k, v in self.model.encoder.module[4].named_modules():
                #     if 'bn' in k:
                #         for para in v.parameters():
                #             para.requires_grad = True

                # self.model.encoder.module[1].weight.requires_grad = True
                # self.model.encoder.module[1].bias.requires_grad = True
                try:
                    self.optimizer = self.optimizer([{'params': self.model.classifier.parameters()}], learning_rate,
                ...
                print('for conv-diff we only train the classifier!!!')
                # model_state = torch.load(fine_tune['pre_trained_model'], map_location=self.device)['model_state_dict']
                #
                # self.model.load_state_dict(model_state)
                # self.model.module.dym_dfn_encoder.bn1.weight.requires_grad = True
                # self.model.module.dym_dfn_encoder.bn1.bias.requires_grad = True

                try:
                    self.optimizer = self.optimizer([{'params': self.model.img_sbn_classifier.parameters()},
                                                     {'params': self.model.dym_dfn_classifier.parameters()},
                                                     ], learning_rate,
                                                    weight_decay=optim_dict['weight_decay'])
                except AttributeError:
                    self.optimizer = self.optimizer([{'params': self.model.module.img_sbn_classifier.parameters()},
                                                     {'params': self.model.module.dym_dfn_classifier.parameters()},
                                                     {'params': self.model.module.avg_weights},
                                                     # {'params': self.model.module.dym_dfn_encoder.bn1.parameters()}
                                                     # {'params': self.model.module.img_conv_lstm.parameters()},
                                                     # {'params': self.model.module.dym_conv_lstm.parameters()}
                                                     # {'params': self.model.module.dym_dfn_encoder.parameters(), 'lr': 1.0e-5},
                                                     # {'params': self.model.module.img_sbn_encoder.parameters(), 'lr': 1.0e-5},
                                                     ], learning_rate, weight_decay=optim_dict['weight_decay'])

        elif fine_tune['stage'] == 'all':
            # # after stage none, fine tune all layers
            # model_state = torch.load(fine_tune['pre_trained_model'], map_location=self.device)['model_state_dict']
            # self.model.load_state_dict(model_state)
            print('we are fine tuning all layers!!!')
            if 'cnn-diff' not in self.model_name:

                self.optimizer = self.optimizer([{'params': self.model.module.layer4.parameters(), 'lr': 5.0e-5},
                                                {'params': self.model.module.classifier.parameters()}], learning_rate,
                                                weight_decay=optim_dict['weight_decay'])
            else:
                self.optimizer = self.optimizer([{'params': self.model.module.img_sbn_encoder.parameters(), 'lr': 5.0e-5},
                                                 {'params': self.model.module.dym_dfn_encoder.parameters(), 'lr': learning_rate},
                                                 {'params': self.model.module.img_sbn_classifier.parameters()},
                                                 {'params': self.model.module.dym_dfn_classifier.parameters()}], learning_rate,
                                                weight_decay=optim_dict['weight_decay'])

        elif fine_tune['stage'] == 'last':
            # fine_tune classifier for different length img sequence
            model_state = torch.load(fine_tune['pre_trained_model'], map_location=self.device)['model_state_dict']
            self.model.load_state_dict(model_state)

            print('we are using pre-trained classifier!!!')

            if self.model_name != 'cnn-diff' and self.model_name != 'cnn-diff-p':
                for para in self.model.encoder.parameters():
                    para.requires_grad = False

                self.optimizer = self.optimizer([{'params': self.model.combiner.parameters()}], learning_rate,
                                                weight_decay=optim_dict['weight_decay'])
            else:
                print('for conv-diff we only train the classifier!!!')
                self.optimizer = self.optimizer([{'params': self.model.img_sbn_classifier.parameters()},
                                                 {'params': self.model.dym_dfn_classifier.parameters()}], learning_rate,
                                                weight_decay=optim_dict['weight_decay'])
        else:
            raise ValueError
