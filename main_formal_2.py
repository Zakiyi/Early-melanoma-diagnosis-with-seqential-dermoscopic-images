...
from misc import plot_progress, plot_pred
...
def Ranking_Loss(input, target, margin=(0.05, -0.01), p_index=None):
    """
    :param input: list[N, N, N....]
    :param target: N
    :return:
...
def train(cfg, model, train_dataloader, optimizer, loss_fn, epoch=0):
    ...
    for data in train_dataloader:
        ...
        # tmp_preds = torch.cat(temporal_scores, dim=1)
        # tmp_preds = tmp_preds * data['image']['p_index'][:, :tmp_preds.shape[-1]].to(device)
        # tmp_preds = tmp_preds.sigmoid()
        # plot_pred(tmp_preds[data['target'] == 1, :], tmp_preds[data['target'] == 0, :],
        #               output_dir=os.path.join(cfg['training']['result_dir'], "train_preds.png"))
        ...
        if isinstance(outputs, list) or isinstance(outputs, tuple):
            if len(outputs) > 1:
                ...
                # margin_loss = Margin_Loss(spatial_scores, data['target'].to(device), p_index=data['image']['p_index'].to(device))
...
def val(cfg, model, val_dataloader, loss_fn, epoch=0, teacher_model=None, m=(0.1, -0.1)):
    ...
    with torch.no_grad():
        for data in val_dataloader:
            ...
            # tmp_preds = tmp_preds.sigmoid()
            # plot_pred(tmp_preds[data['target'] == 1, :], tmp_preds[data['target'] == 0, :],
            #               output_dir=os.path.join(cfg['training']['result_dir'], "val_preds.png"))
            ...
            if isinstance(outputs, list) or isinstance(outputs, tuple):
                if len(outputs) > 1:
                    ...
                    # val_loss = m[2]*rank_loss + m[4]*margin_loss
                else:
...
def main(cfg):
    ...
    train_augmentor = Augmentations_diff(train_aug_parameters, color_recalibration=True, padding_mode='normal')
    val_augmentor = Augmentations_diff(val_aug_parameters, color_recalibration=True, padding_mode='normal')
    ...
    if cfg['ten_crop_test']:
        ...
        test_augmentor = Augmentations_diff(test_aug_parameters, test_mode=True, color_recalibration=True, padding_mode='normal')
...
if __name__ == '__main__':
    ...
    teacher_models = '/home/zyi/MedicalAI/Skin_lesion_prognosis/run_exp/cnn-diff-hc_MIC/New_exps_score_wo_sftx/bce_0.8_0.2_rank(margin)_distill_1.6_1.2_score_base'
        ...
        for seq_length in [4]:
            ...
            for fold in [4, 3]:
                ...
                if not os.path.exists(config_file['training']['resume_path']):
                    ...
                    config_file['training']['result_dir'] = os.path.join('/home/zyi/My_disk/medical_AI/cnn-diff-hc-sa/baseline/w',
                                                                         'round_{}'.format(round),
                                                                         'seq_length_{}'.format(
