...
def restore_vgg(sess, path):
    ...
    # caffe_weights = np.load(caffe_weights_fn).item()
    var_to_restore = []
    vgg_var_list = [l.split("\n")[0] for l in open('/home/zyi/MedicalAI/Skin_lesion_prognosis/registration/helper/vgg_var.txt').readlines()]
...
def restore_alexnet(sess, path):
    ...
    var_to_restore = []
    nosplit_list = [l.split("\n")[0] for l in open('/home/zyi/MedicalAI/Skin_lesion_prognosis/registration/helper/nosplit_var.txt').readlines()]
    split_list = [l.split("\n")[0] for l in open('/home/zyi/MedicalAI/Skin_lesion_prognosis/registration/helper/split_var.txt').readlines()]
    var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    for var in var_list:
        if len(var.op.name.split("/")) != 3:
            # print('No load %s' %var.op.name)
