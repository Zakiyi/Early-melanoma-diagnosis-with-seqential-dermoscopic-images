...
def load_classifier_dict(classifier, model_dir):
    model = torch.load(model_dir, map_location='cpu')['model_state_dict']
...
class TDNHCRE(nn.Module):
    def __init__(self, in_channel=3, n_class=1, seq_length=4, pre_trained_clss=None):
        ...
        self.exit_layer = nn.RNN(32, 16, 1)
        self.clss_layer = nn.Linear(16, 1)
    ...
    def forward(self, img_seq, diff_seq, p_index=None):
        ...
        if Time_step > 1:
            ...
            exit_features.append(self.rnn_dropout(torch.cat([x_0, torch.zeros_like(x_0)], dim=1)))
            ...
            for i in range(Time_step-1):
                ...
                exit_features.append(self.rnn_dropout(torch.cat([x_i, x_d], dim=1)))

            exit_features = torch.stack(exit_features, dim=0)

            out, _ = self.exit_layer(exit_features)

            for j in range(out.shape[0]):
                exit_feature_scores.append(self.clss_layer(out[j, ...]))
            ...
            # final_pred = [img_sbn_scores, dym_dfn_scores, average_scores]
        ...
        return final_pred, spatial_scores, temporal_scores, exit_feature_scores


if __name__ == '__main__':
    input_seq = torch.rand(3, 4, 3, 320, 320)
    p_index = torch.ones(3, 4)
    resnet = TDNHCRE(seq_length=4)
