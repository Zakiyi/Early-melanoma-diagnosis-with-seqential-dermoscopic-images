...
class TDNHCSA(nn.Module):
    def __init__(self, in_channel=3, n_class=1, seq_length=4, pre_trained_clss=None):
        ...
        self.clss_layer = nn.Linear(48, 1)
    ...
    def forward(self, img_seq, diff_seq, p_index=None):
        ...
        if Time_step > 1:
            ...
            exit_features_att = self.attention_block(exit_features)  # NTC

            for j in range(exit_features.shape[1]):
                exit_feature_scores.append(self.clss_layer(exit_features_att[:, j, :]))
            ...
            raise ValueError
        return final_pred, exit_features, exit_features_att, exit_feature_scores
